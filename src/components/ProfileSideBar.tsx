...
const ProfileSideBar = ({ chngMenu }: ProfileSideBarProps) => {
    ...
    return (
        <>
            <div className={` ${pathname === "/main" ? "w-[545px] relative overflow-y-scroll h-auto" : "hidden sm:inline-block sm:w-[545px] h-auto"} `}>

                <div className="bg-[#3E3E3E] chat-widget flex ">
                    <div className="sm:flex mx-auto sm:m-0 ">
                        <img src={user.img ? user.img : "/bilan.svg"} alt="profile" width={80} className='border-[#07F] border-2 rounded-[50px] m-auto sm:m-0 cursor-pointer' onClick={() => chngMenu(false)} />
                ...
                <div className="bg-[#373737] h-screen flex flex-col justify-between pb-[100px]">
                    {openSettings && <ChangeName setGroup={setOpenGroup} setSettings={setOpenSettings} setMenu={chngMenu} />}
